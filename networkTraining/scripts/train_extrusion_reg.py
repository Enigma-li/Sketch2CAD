...
# TensorBoard
def collect_vis_img(logit_fh, logit_curve, user_stroke, cnormal, cdepth, face_heatmap, curve,
    ...
    with tf.name_scope(scope) as _:
        ...
        curve_proto = tf.summary.image(curve_fn, curve, hyper_params['nbDispImg'])

    return [logit_fh_proto, logit_mg_proto, userStroke_proto, normal_proto, depth_proto, face_map_proto,
            curve_proto]
...
def train_procedure(net, train_records):
    ...
    with tf.name_scope('divide_data'):
        ...
        gpu_curve = tf.split(gtoffCurve_input, nb_gpus, axis=0)
        ...
        for gpu_id in range(nb_gpus):
            with tf.device('/gpu:%d' % gpu_id):
                with tf.name_scope('tower_%s' % gpu_id) as _:
                    # network forward
                    logit_fh, logit_curve, _ = net.load_extrusion_reg_net(gpu_user_strokes[gpu_id],
                                                                          gpu_cnormal[gpu_id],
                                                                          gpu_cdepth[gpu_id],
                                                                          hyper_params['rootFt'],
                                                                          is_training=True)
                    ...
                    # training loss
                    train_loss, train_fh_loss, train_curve_loss, train_real_fh_loss, \
                    train_real_curve_loss = loss(logit_fh,
                                                 logit_curve,
                                                 gpu_fh[gpu_id],
                                                 gpu_curve[gpu_id],
                    ...
                    tower_curve_losses.append(train_curve_loss)
                    tower_abs_fh_losses.append(train_real_fh_loss)
                    tower_abs_curve_losses.append(train_real_curve_loss)
    ...
    train_diff_mg_proto = tf.summary.scalar('Training_CurveL2Loss', averaged_losses[2])
    train_diff_real_fn_proto = tf.summary.scalar('Training_FhL1Loss', averaged_losses[3])
    train_diff_real_mg_proto = tf.summary.scalar('Training_CurveL1Loss', averaged_losses[4])
    ...
    proto_list.append(train_diff_mg_proto)
    proto_list.append(train_diff_real_fn_proto)
    proto_list.append(train_diff_real_mg_proto)
...
def validation_procedure(net, val_records):
    ...
    # Loss
    val_loss, val_fh_loss, val_curve_loss, val_real_fh_loss, val_real_curve_loss = loss(logit_fh,
                                                                                        logit_curve,
                                                                                        gtFH_input,
                                                                                        gtoffCurve_input,
                                                                                        userStroke_input,
    ...
    return merged_val, val_loss, val_fh_loss, val_curve_loss, val_real_fh_loss, val_real_curve_loss, \
           [user_strokes, context_normal, context_depth, face_heatm, off_curve]
...
def train_net():
    ...
    val_proto, val_loss, val_fh_loss, val_curve_loss, val_real_fh_loss, val_real_curve_loss, val_inputList \
        = validation_procedure(net, val_records)
    ...
    valid_curve_loss = tf.placeholder(tf.float32, name='mval_curve_loss')
    valid_curve_loss_proto = tf.summary.scalar('Validating_CurveL2Loss', valid_curve_loss)
    ...
    valid_real_curve_loss = tf.placeholder(tf.float32, name='mval_real_curve_loss')
    valid_real_curve_loss_proto = tf.summary.scalar('Validating_CurveL1Loss', valid_real_curve_loss)
    valid_value_merge = tf.summary.merge([valid_loss_proto, valid_fh_loss_proto, valid_curve_loss_proto,
                                          valid_real_fh_loss_proto, valid_real_curve_loss_proto])
        ...
        for titr in range(hyper_params['maxIter']):
            ...
            if titr % hyper_params['exeValStep'] == 0:
                ...
                for eitr in range(num_eval_itr):
                    ...
                    if eitr == idx:
                        val_merge, cur_v_loss, cur_vfh_loss, cur_vcurve_loss, cur_real_fh_loss, cur_real_curve_loss \
                            = sess.run(
                            [val_proto, val_loss, val_fh_loss, val_curve_loss, val_real_fh_loss, val_real_curve_loss],
                            feed_dict={'user_input:0': val_real_input[0],
                                       ...
                                       'gtoffCurve_input:0': val_real_input[4]})
                        train_writer.add_summary(val_merge, titr)
                    else:
                        cur_v_loss, cur_vfh_loss, cur_vcurve_loss, cur_real_fh_loss, cur_real_curve_loss = sess.run(
                            [val_loss, val_fh_loss, val_curve_loss, val_real_fh_loss, val_real_curve_loss],
                            feed_dict={'user_input:0': val_real_input[0],
                                       'normal_input:0': val_real_input[1],
                                       'depth_input:0': val_real_input[2],
                                       'gtFH_input:0': val_real_input[3],
            ...
            t_summary, _, t_loss = sess.run([train_summary, train_step, train_loss],
                                            feed_dict={'user_input:0': train_real_input[0],
                                                       ...
                                                       'gtoffCurve_input:0': train_real_input[4]})
