...
# Loss term
def loss(logit_fh, logit_bc, fh, bc, useStroke, cnormal, cdepth, scope='loss'):
    with tf.name_scope(scope) as _:
        ...
        stroke_mask = full_one - useStroke
        ...
        return total_loss, fh_loss, bc_loss, real_fh_loss, real_bc_loss, logit_fh, logit_bc, fh, bc, useStroke, \
               cnormal, cdepth, stroke_mask
...
def test_net():
    ...
    test_total_loss, test_fh_loss, test_bc_loss, test_real_fh_loss, test_real_bc_loss, test_pred_fh, test_pred_bc, \
    test_gt_fh, test_gt_bc, test_userStroke, test_cnormal, test_cdepth, test_strokeMask, \
    test_inputList = test_procedure(net, test_records)
    ...
    with tf.Session(config=config) as sess:
        ...
        tf.train.write_graph(sess.graph_def, out_net_dir,
                             "Bevel_Opt_Reg.pbtxt", as_text=True)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                t_loss, t_fh_loss, t_bc_loss, t_real_fh_loss, t_real_bc_loss, t_gt_fh, t_pred_fh, t_gt_bc, t_pred_bc, \
                t_userStroke, t_cnormal, t_cdepth, t_strokeMask = sess.run(
                    [test_total_loss, test_fh_loss, test_bc_loss, test_real_fh_loss, test_real_bc_loss, test_gt_fh,
                     test_pred_fh, test_gt_bc, test_pred_bc, test_userStroke, test_cnormal,
                     test_cdepth, test_strokeMask],
                    feed_dict={'user_input:0': test_realInput[0],
                               ...
                               'gtBaseCurve_input:0': test_realInput[4]})
                ...
                avg_bc_loss += t_bc_loss
                avg_real_fh_loss += t_real_fh_loss
                avg_real_bc_loss += t_real_bc_loss
                test_logger.info('Test case {}, total loss: {}, fh: {}, bc: {}, rfh: {}, rbc: {},'
                                 'fIOU: {}, bAcc: {}'.format(titr,
                                                             t_loss,
                                                             t_fh_loss,
                                                             t_bc_loss,
                                                             t_real_fh_loss,
                ...
                if titr < 50:
                    ...
                    out_gt_bc = t_gt_bc
                    cv2.imwrite(fn2, out_gt_bc)

                    out_pred_bc = t_pred_bc
                    cv2.imwrite(fn3, out_pred_bc)
            ...
            avg_real_bc_loss /= titr
            avg_fIOU /= titr
            avg_bAcc /= titr
            test_logger.info(
                "Finish test model, average total loss: {}, fh: {}, bc: {}, rfh: {}, rbc: {},"
                "fIOU: {}, bAcc: {}".format(avg_loss,
                                            avg_fh_loss,
                                            avg_bc_loss,
                                            avg_real_fh_loss,
                                            avg_real_bc_loss,
